...
impl Draw for RectFull {
    fn display(&self) {
        crate::screen_util::bagl_hal_draw_rect(1, self.pos.0, self.pos.1, self.width, self.height);
    }

    fn erase(&self) {
        crate::screen_util::bagl_hal_draw_rect(0, self.pos.0, self.pos.1, self.width, self.height);
    }
}
